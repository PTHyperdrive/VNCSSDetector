use std::io::Write;
use std::path::Path;
/// Installer for the Msm8916 hotspot.
///
/// Installation process:
/// 1. Use curl to activate USB debugging backdoor
/// 2. Wait for device reboot and ADB availability
/// 3. Use ADB to install vncssdetector files
/// 4. Modify startup script to launch vncssdetector on boot
...
async fn run_install(admin_ip: String) -> Result<()> {
    ...
    echo!("Installing VNCSSdetector files... ");
    install_vncssdetector_files(&mut adb_device).await?;
    println!("ok");
    ...
    println!("Then access at: http://{admin_ip}:8080");
...
async fn wait_for_adb() -> Result<ADBUSBDevice> {
    ...
    loop {
        ...
        // MSM8916 USB vendor and product IDs.
...
async fn install_vncssdetector_files(adb_device: &mut ADBUSBDevice) -> Result<()> {
    // Create vncssdetector directory
    let mut buf = Vec::<u8>::new();
    adb_device.shell_command(&["mkdir", "-p", "/data/vncssdetector"], &mut buf)?;

    // Remount system as writable
    adb_device.shell_command(&["mount", "-o", "remount,rw", "/system"], &mut buf)?;

    // Install vncssdetector daemon binary with verification
    ...
    // Install config file
    let config_content = crate::CONFIG_TOML.replace("#device = \"orbic\"", "device = \"msm8916\"");
    let mut config_data = config_content.as_bytes();
    adb_device.push(&mut config_data, &"/data/vncssdetector/config.toml")?;

    // Make daemon executable
    let mut buf = Vec::<u8>::new();
    adb_device.shell_command(
        &["chmod", "755", "/data/vncssdetector/vncssdetector-daemon"],
        &mut buf,
    )?;
...
async fn modify_startup_script(adb_device: &mut ADBUSBDevice) -> Result<()> {
    ...
    adb_device.pull(&"/system/bin/initmifisservice.sh", &mut script_content)?;

    // Convert to string and add our line
    let mut script_str = String::from_utf8_lossy(&script_content).into_owned();

    // Add vncssdetector startup line if not already present
    let vncssdetector_line = "/data/vncssdetector/vncssdetector-daemon /data/vncssdetector/config.toml &\n";
    if !script_str.contains("/data/vncssdetector/vncssdetector-daemon") {
        script_str.push_str(vncssdetector_line);
    }
    ...
    let mut buf = Vec::<u8>::new();
    adb_device.shell_command(
        &["chmod", "755", "/system/bin/initmifisservice.sh"],
        &mut buf,
    )?;
