...
use crate::{CONFIG_TOML, VNCSSdetector_DAEMON_INIT};
...
pub async fn install() -> Result<()> {
    ...
    println!("done");
    echo!("Installing vncssdetector... ");
    let mut adb_device = setup_vncssdetector(adb_device).await?;
    println!("done");
    echo!("Testing vncssdetector... ");
    test_vncssdetector(&mut adb_device).await?;
    println!("done");
    Ok(())
}
...
pub async fn shell() -> Result<()> {
    println!(
        "WARNING: The orbic USB installer is likely to go away in a future version of VNCSSdetector. Consider using ./installer util orbic-start-telnet instead."
    );
...
async fn setup_vncssdetector(mut adb_device: ADBUSBDevice) -> Result<ADBUSBDevice> {
    let vncssdetector_daemon_bin = include_bytes!(env!("FILE_VNCSSDETECTOR_DAEMON"));

    adb_at_syscmd(&mut adb_device, "mkdir -p /data/vncssdetector").await?;
    install_file(
        &mut adb_device,
    ...
    install_file(
        &mut adb_device,
        "/data/vncssdetector/config.toml",
        CONFIG_TOML
    ...
    install_file(
        ...
        VNCSSdetector_DAEMON_INIT.as_bytes(),
    )
    .await?;
    install_file(
        &mut adb_device,
    ...
    .await?;
    adb_at_syscmd(&mut adb_device, "chmod 755 /etc/init.d/vncssdetector_daemon").await?;
...
/// Test vncssdetector on the device over adb without forwarding.
pub async fn test_vncssdetector(adb_device: &mut ADBUSBDevice) -> Result<()> {
    ...
    bail!("timeout reached! failed to reach vncssdetector, something went wrong :(")
}
