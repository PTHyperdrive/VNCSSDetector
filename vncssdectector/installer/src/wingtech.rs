...
async fn wingtech_run_install(admin_ip: String, admin_password: String) -> Result<()> {
    ...
    telnet_send_command(addr, "mkdir -p /data/vncssdetector", "exit code 0", true).await?;
    println!("ok");
    ...
    telnet_send_file(
        ...
        crate::VNCSSdetector_DAEMON_INIT.as_bytes(),
        true,
    )
    .await?;
    telnet_send_command(
    ...
    .await?;
    println!("ok");
    println!("vncssdetector is running at http://{admin_ip}:8080");
