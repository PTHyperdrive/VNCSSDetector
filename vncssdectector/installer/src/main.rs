...
pub static VNCSSdetector_DAEMON_INIT: &str = include_str!("../../dist/scripts/vncssdetector_daemon");
...
enum Command {
    /// Install vncssdetector on the Orbic RC400L using the legacy USB+ADB-based installer.
    OrbicUsb(InstallOrbic),
    /// Install vncssdetector on the Orbic RC400L or Moxee Hotspot via network.
    #[clap(alias = "orbic-network")]
    Orbic(OrbicNetworkArgs),
    /// Install vncssdetector on the TMobile TMOHS1.
    Tmobile(TmobileArgs),
    /// Install vncssdetector on the Msm8916.
    Msm8916(Msm8916Args),
    /// Install vncssdetector on a PinePhone's Quectel modem.
    Pinephone(InstallPinephone),
    /// Install vncssdetector on the TP-Link M7350.
    Tplink(InstallTpLink),
    /// Install vncssdetector on the Wingtech CT2MHS01.
...
enum UtilSubCommand {
    ...
    /// Root the Msm8916 and launch adb.
    Msm8916StartAdb(Msm8916Args),
...
#[derive(Parser, Debug)]
struct Msm8916Args {
    /// IP address for Msm8916 admin interface, if custom.
...
async fn run() -> Result<(), Error> {
    ...
    match command {
        Command::Tmobile(args) => tmobile::install(args).await.context("Failed to install vncssdetector on the Tmobile TMOHS1. Make sure your computer is connected to the hotspot using USB tethering or WiFi.")?,
        Command::Msm8916(args) => msm8916::install(args).await.context("Failed to install vncssdetector on the Msm8916. Make sure your computer is connected to the hotspot using USB.")?,
        Command::Tplink(tplink) => tplink::main_tplink(tplink).await.context("Failed to install vncssdetector on the TP-Link M7350. Make sure your computer is connected to the hotspot using USB tethering or WiFi.")?,
        Command::Pinephone(_) => pinephone::install().await
            .context("Failed to install vncssdetector on the Pinephone's Quectel modem")?,
        Command::OrbicUsb(_) => orbic::install().await.context("\nFailed to install vncssdetector on the Orbic RC400L (USB installer)")?,
        Command::Orbic(args) => orbic_network::install(args.admin_ip, args.admin_username, args.admin_password).await.context("\nFailed to install vncssdetector on the Orbic RC400L")?,
        Command::Wingtech(args) => wingtech::install(args).await.context("\nFailed to install vncssdetector on the Wingtech CT2MHS01")?,
        Command::Util(subcommand) => match subcommand.command {
            ...
            UtilSubCommand::Msm8916StartAdb(args) => msm8916::activate_usb_debug(&args.admin_ip).await.context("\nFailed to activate USB debug on the Msm8916")?,
