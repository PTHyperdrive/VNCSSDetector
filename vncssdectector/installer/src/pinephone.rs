...
use crate::orbic::test_vncssdetector;
...
pub async fn install() -> Result<()> {
    ...
    adb.run_command(&["mkdir", "-p", "/data/vncssdetector"], "exit code 0")?;

    let vncssdetector_daemon_bin = include_bytes!(env!("FILE_VNCSSDETECTOR_DAEMON"));
    adb.install_file("/data/vncssdetector/vncssdetector-daemon", vncssdetector_daemon_bin)?;
    adb.install_file(
    ...
    test_vncssdetector(&mut adb).await?;
    println!("ok");
    println!("vncssdetector is running on the modem. Use adb to access the web interface.");
