...
pub async fn install(
    ...
    let Some(admin_password) = admin_password else {
        ...
        eprintln!(
            "Refer to the official documentation at https://efforg.github.io/vncssdetector/ for how to find the right value."
        );
        eprintln!();
        eprintln!(
    ...
    setup_vncssdetector(&admin_ip).await
}
...
async fn setup_vncssdetector(admin_ip: &str) -> Result<()> {
    let addr = SocketAddr::from_str(&format!("{}:24", admin_ip))?;
    ...
    telnet_send_command(addr, "mkdir -p /data/vncssdetector", "exit code 0", false).await?;
    ...
    telnet_send_command(
        addr,
        "chmod +x /data/vncssdetector/vncssdetector-daemon",
        "exit code 0",
        false,
    )
    .await?;
