...
function setup_vncssdetector {
	write-host "installing vncssdetector..."
	_serial "AT+SYSCMD=mkdir -p /data/vncssdetector" | Out-Host
	_adb_push "config.toml.in" "/tmp/config.toml" | Out-Null
	_serial "AT+SYSCMD=mv /tmp/config.toml /data/vncssdetector" | Out-Host
	_adb_push "vncssdetector-daemon-orbic/vncssdetector-daemon" "/tmp/vncssdetector-daemon" | Out-Null
	_serial "AT+SYSCMD=mv /tmp/vncssdetector-daemon /data/vncssdetector" | Out-Host
	_adb_push "scripts/vncssdetector_daemon" "/tmp/vncssdetector_daemon" | Out-Null
	_serial "AT+SYSCMD=mv /tmp/vncssdetector_daemon /etc/init.d/vncssdetector_daemon" | Out-Host
	...
	_serial "AT+SYSCMD=chmod 755 /data/vncssdetector/vncssdetector-daemon" | Out-Host
	_serial "AT+SYSCMD=chmod 755 /etc/init.d/vncssdetector_daemon" | Out-Host
...
function test_vncssdetector {
	...
	write-host "checking for vncssdetector server..." -nonewline
	$seconds = 0
	do {
		try {
		...
		if ($resp.statuscode -eq 200) {
			write-host "success!"
			write-host "you can access vncssdetector at $($URL)"
			return
		}
		start-sleep 1
		$seconds = $seconds + 1
	} until ($seconds -eq 30)
	write-host "timeout reached! failed to reach vncssdetector url $($URL), something went wrong :("
}
