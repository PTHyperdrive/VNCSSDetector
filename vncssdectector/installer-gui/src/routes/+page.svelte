<script lang="ts">
    ...
    async function run_installer(event: Event) {
        ...
        installerOutput = '';
        try {
            await invoke('install_vncssdetector', { args: installerArgs });
        } catch (error) {
            installerOutput +=
                'VNCSSdetector GUI installer encountered an internal error. Error was:\n';
...
<div class="p-4 xl:px-8 bg-vncssdetector-blue drop-shadow flex flex-row justify-between items-center">
    <!-- https://www.w3.org/WAI/tutorials/images/decorative/ -->
    <img src="/vncssdetector_text.png" alt="" class="h-10 xl:h-12" />
    <div class="flex flex-row gap-4">
        <a
            class="flex flex-row gap-1 group"
            href="https://github.com/EFForg/vncssdetector/issues"
            target="_blank"
        >
        ...
        <a
            class="flex flex-row gap-1 group"
            href="https://efforg.github.io/vncssdetector/"
            target="_blank"
        >
