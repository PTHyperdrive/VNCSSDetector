# VNCSSdetector GUI Installer

This directory contains experimental work on a VNCSSdetector GUI installer based on [Tauri](https://tauri.app/).
...
### VNCSSdetector CLI Installer

The VNCSSdetector GUI installer currently just bundles and wraps the CLI VNCSSdetector installer. When building the GUI installer, the CLI installer needs to be built and available for bundling. By default it assumed the installer is present in the repo's `target` directory at either `debug/installer` or `release/installer` depending on whether you're doing a debug or release build of the GUI installer.
