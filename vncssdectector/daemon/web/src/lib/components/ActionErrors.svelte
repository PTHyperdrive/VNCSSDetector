    ...
    >
        <div class="flex flex-row justify-between">
            <span class="text-xl font-bold mb-2 mr-5 flex flex-row items-center gap-1 text-red-600">
                ...
                Lỗi Khi Thực Hiện Hành Động {current_error.times > 1 ? `x${current_error.times}` : ''}
            </span>
            <div class="flex items-center mb-2">
                {#if action_errors.length > 1}
                    <span>{pos + 1}/{action_errors.length}</span>
                    <button title="lỗi trước" aria-label="lỗi trước" onclick={prev_error}>
                        <svg
                            aria-hidden="true"
                            width="24"
                            height="24"
                    ...
                    <button title="lỗi tiếp" aria-label="lỗi tiếp" onclick={next_error}>
                        <svg
                            aria-hidden="true"
                            width="24"
                            height="24"
                ...
                {/if}
                <button title="xóa lỗi" aria-label="xóa lỗi" onclick={clear_errors}>
