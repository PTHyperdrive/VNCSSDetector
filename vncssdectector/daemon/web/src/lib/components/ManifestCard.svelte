...
>
    {#if current}
        <div class="flex flex-row justify-between gap-2">
            <span class="text-xl mb-2">Bản Ghi Hiện Tại</span>
            <span class=""
                ><AnalysisStatus
    ...
    <div class="flex flex-col">
        <span class="">Bắt đầu: {date_formatter.format(entry.start_time)}</span>
        <span class=""
            >Tin nhắn cuối: {(entry.last_message_time &&
    ...
    <div class="flex flex-row justify-between lg:justify-end gap-1 mt-2 overflow-x-auto">
        ...
        {:else}
            <DeleteButton
                prompt={`Bạn có chắc chắn muốn xóa bản ghi ${entry.name}?`}
