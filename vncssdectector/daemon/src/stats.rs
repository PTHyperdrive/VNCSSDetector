...
use log::error;
use vncssdetector::{Device, util::RuntimeMetadata};
use serde::Serialize;
    ...
    pub async fn new(qmdl_path: &str, device: &Device) -> Result<Self, String> {
        Ok(Self {
            ...
            battery_status: match get_battery_status(device).await {
                ...
                Err(VNCSSdetectorError::FunctionNotSupportedForDeviceError) => None,
                Err(err) => {
...
impl DiskStats {
    ...
    // the QMDL file.
    pub async fn new(qmdl_path: &str, device: &Device) -> Result<Self, String> {
        // Msm8916 needs to be told to use the busybox df specifically
        let mut df_cmd: Command;
        if matches!(device, Device::Msm8916) {
...
pub async fn get_log() -> Result<String, (StatusCode, String)> {
    tokio::fs::read_to_string("/data/vncssdetector/vncssdetector.log")
        .await
