use crate::{battery::BatteryState, error::VNCSSdetectorError};

pub async fn get_battery_state() -> Result<BatteryState, VNCSSdetectorError> {
    ...
    if !uci_battery.status.success() {
        return Err(VNCSSdetectorError::BatteryLevelParseError);
    }

    if !uci_plugged_in.status.success() {
        return Err(VNCSSdetectorError::BatteryPluggedInStatusParseError);
    }

    let uci_battery = String::from_utf8_lossy(&uci_battery.stdout)
        .trim_end()
        .parse()
        .map_err(|_| VNCSSdetectorError::BatteryLevelParseError)?;
