...
async fn is_plugged_in_from_file(path: &Path) -> Result<bool, VNCSSdetectorError> {
    match tokio::fs::read_to_string(path)
        .await
        .map_err(VNCSSdetectorError::TokioError)?
        .chars()
        .next()
    {
        Some('0') => Ok(false),
        Some('1') => Ok(true),
        _ => Err(VNCSSdetectorError::BatteryPluggedInStatusParseError),
    }
}

async fn get_level_from_percentage_file(path: &Path) -> Result<u8, VNCSSdetectorError> {
    tokio::fs::read_to_string(path)
        .await
        .map_err(VNCSSdetectorError::TokioError)?
        .trim_end()
        .parse()
        .or(Err(VNCSSdetectorError::BatteryLevelParseError))
}

pub async fn get_battery_status(device: &Device) -> Result<BatteryState, VNCSSdetectorError> {
    Ok(match device {
        ...
        _ => return Err(VNCSSdetectorError::FunctionNotSupportedForDeviceError),
    })
}
...
pub fn run_battery_notification_worker(
    ...
    task_tracker.spawn(async move {
        ...
        let mut triggered = match get_battery_status(&device).await {
            Err(VNCSSdetectorError::FunctionNotSupportedForDeviceError) => {
            ...
            if !triggered && !status.is_plugged_in && status.level <= LOW_BATTERY_LEVEL {
                notification_channel
                    .send(Notification::new(
                        ...
                        "VNCSSdetector's battery is low".to_string(),
                        None,
                    ))
                    .await
