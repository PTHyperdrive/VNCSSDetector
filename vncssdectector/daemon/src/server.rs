...
use crate::config::{Config, NodeConnectionState, RunningMode, ConnectionStatus};
...
pub struct ServerState {
    ...
    // Node/Swarm mode connection state
    pub connection_state: Arc<RwLock<NodeConnectionState>>,
}
...
pub async fn serve_static(
    ...
    match path {
        "vncssdetector_orca_only.png" => (
            [(header::CONTENT_TYPE, HeaderValue::from_static("image/png"))],
            include_bytes!("../web/build/vncssdetector_orca_only.png"),
        )
            .into_response(),
        "vncssdetector_text.png" => (
            [(header::CONTENT_TYPE, HeaderValue::from_static("image/png"))],
            include_bytes!("../web/build/vncssdetector_text.png"),
        )
            .into_response(),
        "favicon.png" => (
            [(header::CONTENT_TYPE, HeaderValue::from_static("image/png"))],
...
pub async fn get_mode(
    State(state): State<Arc<ServerState>>,
) -> Result<Json<ModeStatusResponse>, (StatusCode, String)> {
    let conn_state = state.connection_state.read().await;
    Ok(Json(ModeStatusResponse {
        running_mode: state.config.running_mode.clone(),
        connection_status: conn_state.status.clone(),
        server_ip: conn_state.server_ip.clone(),
        is_heuristic_passed: conn_state.is_heuristic_passed,
        last_error: conn_state.last_error.clone(),
    }))
}
...
pub async fn set_mode(
    ...
    new_config.running_mode = request.running_mode.clone();
    
    let config_str = toml::to_string_pretty(&new_config).map_err(|err| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            format!("failed to serialize config as TOML: {err}"),
        )
    ...
    Ok((
        StatusCode::ACCEPTED,
        format!("Mode changed to {:?} and triggered restart", request.running_mode),
    ))
}
...
pub async fn set_node_config(
    ...
    // Update config with node settings
    let mut new_config = state.config.clone();
    new_config.server_ip = Some(request.server_ip.clone());
    new_config.api_key = Some(request.api_key.clone());
    ...
    // Update connection state
    {
        ...
        conn_state.server_ip = Some(request.server_ip.clone());
        conn_state.status = ConnectionStatus::Connecting;
    }

    // Trigger daemon restart to apply new connection settings
    state.daemon_restart_token.cancel();
    Ok((
        StatusCode::ACCEPTED,
        "Node config updated and connecting".to_string(),
    ))
}

/// Start heuristic test for Swarm mode verification
pub async fn start_heuristic_test(
    State(state): State<Arc<ServerState>>,
) -> Result<(StatusCode, String), (StatusCode, String)> {
    // Reset heuristic status
    {
        let mut conn_state = state.connection_state.write().await;
        conn_state.is_heuristic_passed = false;
    }
    
    // The heuristic test is "passed" when a warning is generated
    // This is handled by the analysis system - if warning shows, test passes
    Ok((
        StatusCode::OK,
        "Heuristic test started. Waiting for detector warnings...".to_string(),
    ))
}
