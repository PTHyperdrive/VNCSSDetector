...
use diag::{
    ...
    run_diag_read_thread_new, start_recording, stop_recording,
};
use log::{error, info};
use qmdl_store::RecordingStoreError;
use vncssdetector::Device;
use vncssdetector::diag_device::DiagDevice;
use vncssdetector::diag_newdevice::NewDiagDevice;
use stats::get_log;
...
async fn init_qmdl_store(config: &config::Config) -> Result<RecordingStore, VNCSSdetectorError> {
    ...
    if config.debug_mode {
        if store_exists {
            ...
            Err(VNCSSdetectorError::NoStoreDebugMode(
...
) -> JoinHandle<Result<(), VNCSSdetectorError>> {
...
async fn main() -> Result<(), VNCSSdetectorError> {
    env_logger::init();
...
async fn run_with_config(
    ...
    if !config.debug_mode {
        ...
        // Try NewDiagDevice first (for newer Qualcomm chipsets like Snapdragon 855+)
        // If that fails, fall back to the original DiagDevice
        match NewDiagDevice::new(&config.device).await {
            Ok(mut dev) => {
                info!("Using NewDiagDevice (extended support for newer chipsets)");
                dev.config_logs()
                    .await
                    .map_err(VNCSSdetectorError::DiagInitError)?;
                
                info!("Starting Diag Thread (new device)");
                run_diag_read_thread_new(
                    &task_tracker,
                    dev,
                    diag_rx,
                    diag_tx.clone(),
            ...
            Err(new_err) => {
                info!("NewDiagDevice failed: {new_err}, trying original DiagDevice...");
                match DiagDevice::new(&config.device).await {
                    Ok(mut dev) => {
                        info!("Using original DiagDevice");
                        dev.config_logs()
                            .await
                            .map_err(VNCSSdetectorError::DiagInitError)?;
                    ...
                    Err(orig_err) => {
                        error!("Both DiagDevice types failed to initialize");
                        error!("NewDiagDevice error: {new_err}");
                        error!("DiagDevice error: {orig_err}");
                        return Err(VNCSSdetectorError::DiagInitError(orig_err));
                    }
                }
            }
        }
        ...
        let update_ui = match &config.device {
            ...
            Device::Msm8916 => display::msm8916::update_ui,
        };
