...
use vncssdetector::analysis::analyzer::{AnalysisLineNormalizer, AnalyzerConfig, EventType};
use vncssdetector::diag::{DataType, MessagesContainer};
use vncssdetector::diag_device::DiagDevice;
use vncssdetector::diag_newdevice::NewDiagDevice;
use vncssdetector::qmdl::QmdlWriter;
            ...
            if max_type > EventType::Informational {
                ...
                self.notification_channel
                    .send(Notification::new(
                        ...
                        format!("VNCSSdetector has detected a {:?} severity event", max_type),
...
/// Diag read thread for newer Qualcomm chipsets (Snapdragon 855+)
#[allow(clippy::too_many_arguments)]
pub fn run_diag_read_thread_new(
    task_tracker: &TaskTracker,
    mut dev: NewDiagDevice,
    mut qmdl_file_rx: Receiver<DiagDeviceCtrlMessage>,
    qmdl_file_tx: Sender<DiagDeviceCtrlMessage>,
    ui_update_sender: Sender<display::DisplayState>,
    qmdl_store_lock: Arc<RwLock<RecordingStore>>,
            ...
            tokio::select! {
                msg = qmdl_file_rx.recv() => {
                    match msg {
                        ...
                        Some(DiagDeviceCtrlMessage::Exit) | None => {
                            info!("Diag reader thread (new) exiting...");
                            diag_task.stop_current_recording().await;
                            return Ok(())
                        },
                        Some(DiagDeviceCtrlMessage::DeleteEntry { name, response_tx }) => {
                ...
                maybe_container = diag_stream.next() => {
                    match maybe_container.unwrap() {
                        ...
                        Err(err) => {
                            error!("error reading diag device (new): {err}");
                            return Err(err);
                        }
                    }
                }
