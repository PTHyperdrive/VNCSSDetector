...
Building VNCSSdetector from source, either for development or because the install script doesn't work on your system, involves a number of external dependencies. Unless you need to do this, we recommend you use our [compiled builds](https://github.com/EFForg/vncssdetector/releases).

* Install [nodejs/npm](https://docs.npmjs.com/downloading-and-installing-node-js-and-npm), which is required to build VNCSSdetector's web UI
  * Make sure to build the site with `pushd daemon/web && npm install && npm run build && popd` before building VNCSSdetector. If you're working directly on the frontend, `npm run dev` will allow you to test a local frontend with hot-reloading (use `http://localhost:5173` instead of `http://localhost:8080`).
...
- install the cross-compilation target for the device VNCSSdetector will run on:
```sh
...
Now you can root your device and install VNCSSdetector by running:
...
# Use FILE_VNCSSdetector_DAEMON to specify the daemon binary path when using development builds:
FILE_VNCSSdetector_DAEMON=$PWD/target/armv7-unknown-linux-musleabihf/firmware-devel/vncssdetector-daemon cargo run -p installer --bin installer orbic
```
...
* Push the scripts in `scripts/` to `/etc/init.d` on device and make a directory called `/data/vncssdetector` using `adb shell` (and sshell for your root shell if you followed the steps above)
* You also need to copy `config.toml.in` to `/data/vncssdetector/config.toml`. Uncomment the `device` line and set the value to your device type if necessary.
* Then run `./make.sh`, which will build the binary, push it over adb, and restart the device. Once it's restarted, VNCSSdetector should be running!
