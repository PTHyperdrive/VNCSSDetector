...
VNCSSdetector has been tested on MSM8916 devices with firmware supporting USB debugging backdoor access. It is not certain whether all of the sticks that use this board will be compatible with the automated installer, or even with any alternative manual installation method. Please consider sharing your device's firmware version and hardware information [here](https://github.com/EFForg/vncssdetector/discussions/479) to help improve compatibility.
...
The MSM8916 supports various LTE bands depending on the specific hardware revision and carrier customization. Check your device specifications for the exact band support.
...
Note: The default IP for MSM8916 is typically `192.168.100.1`; if yours differs, use the `--admin-ip` argument to specify it.
...
Note: Unlike the TMOHS1, the MSM8916 uses solid LED indicators instead of blinking patterns.

## Obtaining a shell
The MSM8916 supports ADB access after the USB debugging backdoor is activated.
...
The MSM8916 uses a unique installation process that activates a hidden USB debugging backdoor.
...
3. Uses ADB to install vncssdetector files and modify the startup script
4. Launches vncssdetector daemon automatically

- The MSM8916 does not symlink busybox for some core system utils, for some reason. Please use `busybox <utility_name>`, e.g. `busybox df -h`.
