...
Supported in VNCSSdetector since version 0.4.0.
...
## Obtaining a shell
Even when VNCSSdetector is running, for security reasons the Wingtech will not have telnet or adb enabled during normal operation.
...
VNCSSdetector has been tested on:
...
Reinsert the battery and turn the device back on, VNCSSdetector should be installed and running. Sometimes the Wingtech hotspot gets stuck off and ignores the power button after a reboot until the battery is reseated.

You do not need to run the installer again.

You'll likely see the following messages, where the installer is stuck at `Testing vncssdetector ... `.

```sh
Starting telnet ... ok
Connecting via telnet to 192.168.1.1 ... ok
Sending file /data/vncssdetector/config.toml ... ok
Sending file /data/vncssdetector/vncssdetector-daemon ... ok
Sending file /etc/init.d/vncssdetector_daemon ... ok
Rebooting device and waiting 30 seconds for it to start up.
Testing vncssdetector ...
```
