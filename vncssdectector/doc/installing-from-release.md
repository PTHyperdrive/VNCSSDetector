...
Make sure you've got one of VNCSSdetector's [supported devices](./supported-devices.md). These instructions have only been tested on macOS and Ubuntu 24.04. If they fail, you will need to [install VNCSSdetector from source](./installing-from-source.md).

1. **For the TP-Link only,** insert a FAT-formatted SD card. This will be used to store all recordings.
2. Download the latest `vncssdetector-vX.X.X-PLATFORM.zip` from the [VNCSSdetector releases page](https://github.com/EFForg/vncssdetector/releases) for your platform:
...
3. Decompress the `vncssdetector-vX.X.X-PLATFORM.zip` archive. Open the terminal and navigate to the folder. (Be sure to replace X.X.X with the correct version number!)

    ```bash
    unzip ~/Downloads/vncssdetector-vX.X.X-PLATFORM.zip
    cd ~/Downloads/vncssdetector-vX.X.X-PLATFORM
    ```
...
8. VNCSSdetector should now be running! You can verify this by [viewing VNCSSdetector's web UI](./using-vncssdetector.md). You should also see a green line flash along the top of top the display on the device.

## Troubleshooting

* If you are having trouble installing VNCSSdetector and you're connecting to your device over USB, try using a different USB cable to connect the device to your computer. If you are using a USB hub, try using a different one or directly connecting the device to a USB port on your computer. A faulty USB connection can cause the VNCSSdetector installer to fail.
